...
class Order(models.Model):
    """Заказ — создаётся при успешной оплате через Stripe"""
    ...
    stripe_payment_intent = models.CharField(max_length=255, blank=True, verbose_name='Stripe Payment Intent')

    # Контактные данные (заполняются из Stripe Session)
    email = models.EmailField(verbose_name='Email покупателя')

    # Финансы (в копейках / гривнах)
    ...
    def __str__(self):
        return f'Заказ #{self.id} — {self.get_status_display()} — {self.total} ₴'
...
class OrderItem(models.Model):
    ...
    # Снимок данных товара в момент покупки (на случай изменения товара)
